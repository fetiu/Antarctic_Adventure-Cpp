/*
	...
	2. element*로 받아오던 오브젝트들을 void형 포인터로 변경해봄. 오류가능성 높음...>>다시 원상 복구
...
void Output::fixOnBoard(const Element* object) {
...
void Output::CircleFade(const int (*nextScreen)[38]) {
	...
	for (int R = 1; R < 23; R++) {
		...
		for (int y = 11 - R; y < 11 + R; y++) {	//y가 시작하는 시점은 중심에서 위로 반지름만큼 간 지점.
			space = floor(R - sqrt(R*R - (y - 10.5)*(y - 10.5)) + 0.5) + 18 - R; //세로로 왼쪽지점의 반원만으로 공간을 계산. 
...
void Output::drawScreen() {
	for (int j = 21; j >= 0; j--)
		for (int i = 0; i < 38; i++) {
			...
			case 4:setColor(0x90); cout << "▒"; break;//구덩이
